module.exports = function (path, log) {
    ...
    function handleOutput(source, data, callback) {
        ...
        for (line in lines) {
            ...
            //var matches = /(?:\(not associated\)\W+)((?:[\dA-Fa-f]{2}\:){5}(?:[\dA-Fa-f]{2}))/.exec(line); //<< ignores the access points (fail)
            //var matches = /((?:[\dA-Fa-f]{2}\:){5}(?:[\dA-Fa-f]{2}))  ((?:[\dA-Fa-f]{2}\:){5}(?:[\dA-Fa-f]{2}))/.exec(line); //<< ignores the access points
            if (matches != null && matches.length > 0) {
                //log(module.name, "STDOUT: '" + matches[1] + "'");
